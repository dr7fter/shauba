...
1. **输入阶段（推荐与批改前）**：
   - 每次生成自适应题组、推送题目或深度批改草稿前，**优先读取仓库根目录 `characteristic.md`**，精准感知学员当前处于 `[🔴 待验证]` 的微观考法漏洞与 `[🟢 已固化]` 的已掌握技能。
   - 推荐题目时，必须优先覆盖 🔴 待验证条目的变式题，严禁向学员重复推送 🟢 已固化的同构低价值题。
2. **输出阶段（批改完成后）**：
   - 批改生成 JSON 战报写入收件箱后，**必须实时同步更新 `characteristic.md`**：
     - 若发现新错误断点：追加 `[🔴 待验证]` 条目并注入精准的「⚡ 神经反射补丁」；
     - 若历史弱项在变式中成功纠偏并自主避坑：将状态升级为 `[🟢 已固化]`；
     - 若历史弱项复发：在条目生命周期中追加本次复发证据与强化建议。
   - **对话报告显式反馈**：在每次给学员的文字总结中，专门列出 **【🌟 本轮战力突破 / 成功改正】** 与 **【⚠️ 本轮新增微观断点与补丁】**。

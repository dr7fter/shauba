...
| 期望公式 | `0.50 + 0.04×(mastery−2) − 2.50×(难度系数−1)`，clamp [0.20, 0.80]，review 模式 +0.06 |
...
**尚未修复**：期望基线仍是固定 0.50，而 performance 的实际分布中心在 1.30（因无 AI 批改时六维走默认值，普通做对即被判 1.35）。自适应锚点（用近 30 次 performance 的中位数替代固定基线）已在方案中设计，待回放验证后再上线——它需要重算历史 ELO，会让分数回撤约 100 分，属独立变更。

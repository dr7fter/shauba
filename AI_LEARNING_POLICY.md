...
1. 读取仓库根目录 `characteristic.md` 学员画像（含王牌武器库、真实细粒度掌握度、三级断点与 48~72 小时复习排期表）、App 提供的 `categoryPath`、历史作答、候选题和用户自然语言需求。
2. 选题优先级：① 48~72 小时间隔复习排期表到期题目 $\to$ ② Level 1 致命考法断点变式 $\to$ ③ 结合学员王牌强项进行主思路与快筛分层。
3. 严格区分“已固化强项”“不稳定考法”“高危断点”与“未实测专题”，严禁盲目得出全章封盘结论。

...
>
> 本文定义已经完成实现与桌面端验收的独立“学习中心”页面。它不替换、不改造、不删除现有 `InsightsView`、`ReviewView`、`MasteryMapView` 或其他已有数据页；前五项稳定能力生产默认开启，旧页面继续保留为回退入口。
...
- **发布状态**：v1.5.0 已发布，但首包因生产 Feature Flag 未注入而隐藏新入口；修复后的前五项稳定能力默认开启，待以 v1.5.1 重新发包。`rankedOnlyEloV1` 与 `friendBroadcastsV1` 继续关闭。
...
| 开关 | 默认 | 启用内容 | 回滚行为 |
|---|---|---|---|
| `learningCenterV1` | on | 显示新页面和入口 | 完全隐藏新入口，旧页面不受影响 |
| `learningEvidenceProjectionV1` | on | 五指标、错题状态与 stable gate 的只读投影 | 显示旧聚合，不计算或写入新学习状态 |
| `lowConfidenceGateV1` | on | 低置信度门控与可视化 | 新诊断只作为旁路提示，不能改变旧结算 |
| `nonPressureBatchGradingV1` | on | 非压力模式批量草稿批改入口 | 隐藏入口，不影响压力模式批改 |
| `shadowRecommendationPlanV1` | on | 四轨推荐影子计算与理由 | 显示旧推荐摘要，不改变 TodayView 队列或写正式计划 |
...
- `nonPressureBatchGradingV1` 已完成验收并在生产构建中默认开启；

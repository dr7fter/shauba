...
/// 完整解答的一步。报告的主轴由它构成，因此 quote 必须能在这道题的题库正解里找到。
#[derive(Debug, Serialize, Deserialize, Clone, Default)]
#[serde(rename_all = "camelCase")]
struct WalkthroughStep {
    #[serde(default)]
    n: Option<i64>,
    /// 动作式小标题（≤12 字），写"做什么"不写"用什么"。
    #[serde(default)]
    title: Option<String>,
    /// 这步为什么这么做；只讲判断依据，不复读公式。
    #[serde(default)]
    prose: Option<String>,
    /// 题库正解中对应片段的逐字子串。前端用它 indexOf 定位并校验来源：
    /// 命中→正常渲染，未命中→灰色标「AI 补充」；null 表示题库正解确实不含此步。
    #[serde(default)]
    quote: Option<String>,
}

/// 完整解答（报告主轴），批改侧按 `.agent/09-批改量规.md` 第九节写作。
#[derive(Debug, Serialize, Deserialize, Clone, Default)]
#[serde(rename_all = "camelCase")]
struct Walkthrough {
    /// 一句话领起，说明本题最省路径是什么。
...
/// 一个断点（次要点）。`secondaryTags` 只有标签没有解释，这一层是它的结构化补充。
#[derive(Debug, Serialize, Deserialize, Clone, Default)]
#[serde(rename_all = "camelCase")]
struct DiagnosisBreakpoint {
    #[serde(default)]
    n: Option<i64>,
    #[serde(default)]
    tag: Option<String>,
    ...
    #[serde(default)]
    why: Option<String>,
    /// 对应 walkthrough.steps 的第几步。
    /// **null 不是缺失，是语义**：错路内的二次错误，正解无对应步——报告据此告诉学员"这条不必深究"。
    #[serde(default)]
    step_ref: Option<i64>,
    #[serde(default)]
...
struct GradingDiagnosis {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    syllabus_tools: Vec<String>,
    /// 完整解答——报告的主轴。批改侧按 `.agent/09-批改量规.md` 第九节写，3–6 步。
    /// 未声明则整个 walkthrough 会在入库时被丢掉、报告退回降级渲染，所以必须显式声明。
    #[serde(default)]
    walkthrough: Option<Walkthrough>,
    /// 多个断点的结构化补充。`secondaryTags` 保留兼容（旧报告的 chips 仍在），
    /// 有本字段时前端优先用它、没有才退回 `secondaryTags` 的裸标签。
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    breakpoints: Vec<DiagnosisBreakpoint>,
}
...
2. 六维证据分 (0–100)：**只填你能从草稿确认的维度，最多 3 个**，其余维度**整个省略，不要填 null 占位**。
   每维必须带 `evidence`（一句话指出草稿哪一行/哪一步支撑这个分）；
   ★ 六维已从报告里下线，它是评分内核的输入，不是给学员看的内容——**不要在六维上展开分析**，
   也不要为了凑满六维去编 evidence。缺维比瞎猜强：评分内核原生支持部分维度。
   `strategyInsight` 若填，另给 `techniqueLevel`（1–5）与 `independentDiscovery`
...
5. `diagnosis`：按量规第五节逐项填实；其中 `walkthrough`（完整解答）按**量规第九节**写、
   `breakpoints`（多断点）按**量规第十节**写——这两节是报告的主轴与右栏，不要当成可选字段跳过。
   **`walkthrough` 是正解的唯一完整处**：`fork.standardPath` 不得复述它的步骤，
   `whyDeadEnd` 只讲原理不讲步骤（可复现性的验收判据已移交给 `walkthrough`）。
   `verdict = correct` 时不要整块填 null，改填三字段固化卡（myEntry + rule.positive + whyItWorked）。
   ...
   **职责移交（2026-09-14）**：`walkthrough` 是正解的唯一完整处——
   「学员只读这一段、不翻解析就能复现正确动作」这条验收判据改由它承担；
   `fork.standardPath` 不再写正解，只在分叉点写"正确的选择是什么"（≤2 句）；
   `whyDeadEnd` 只讲原理、不讲步骤。
   **去重**：同一式子或事实全 JSON 不得出现在 3 个以上字段；myEntry 写第一落笔，
   fork.myPath 写错路后续演进，三者与 walkthrough 均不得互为复读。
...
JSON 必须符合（UTF-8，公式用标准单个反斜杠 LaTeX）。缺 `dimensions` 会退回特征曲线评分；带 1–3 个能确认的维度即可，不要凑满六维：
{{"schemaVersion":1,"kind":"analysis","taskId":"{task_id}","questionId":{question_id},"summary":"战术诊断摘要（含 $LaTeX$ 公式）","verdict":"correct|partial|incorrect|uncertain","earliestError":"最早断点行与数学式（含 $LaTeX$）或 null","errorTags":["瞄准失误|概念盲区|战术绕路 三选一"],"secondaryTags":["次要病因，可空数组"],"weaknessTags":["薄弱知识点"],"advice":"下一步修复动作（含 $LaTeX$ 公式）或 null","betterSolution":"捷径注（四条件齐才写，否则 null）","stepScore":72,"confidence":0.95,"rating":1.00,"dimensions":{{"rigor":{{"score":88,"confidence":0.9,"evidence":"依据草稿步骤（含 $LaTeX$）"}},"speed":{{"score":90,"confidence":0.9,"evidence":"基于实际耗时与基准的比值"}},"strategyInsight":{{"score":58,"confidence":0.8,"evidence":"依据结构识别（含 $LaTeX$）","techniqueLevel":3,"independentDiscovery":"uncertain"}}}},"diagnosis":{{"errorCode":"E-027 或 null","title":"根式换元入口缺失","severity":"L1|L2|L3","myEntry":"学员落笔时的第一个动作（含 $LaTeX$）","whyDeadEnd":"这条路径为什么走不通（讲原理，不讲步骤，含 $LaTeX$）","rule":{{"negation":"看到什么特征时禁止做什么","positive":"该做什么"}},"fork":{{"step":1,"label":"换元选择","myPath":"学员实际走的路径（含 $LaTeX$）","standardPath":"正确路径（含 $LaTeX$）","consequence":"走错之后的后果"}},"acceptance":"一条可判定的验收判据","nextAction":"明天就能做的一条动作","whyItWorked":null,"syllabusTools":["定积分换元公式"],"walkthrough":{{"lead":"本题最省路径是什么（≤40字）","steps":[{{"n":1,"title":"动作式小标题≤12字","prose":"这步为什么这么做，不复读公式","quote":"题库正解中对应片段的逐字子串，无则null"}}]}},"breakpoints":[{{"n":1,"tag":"次要点名称","why":"为什么会错（一句话）","stepRef":2,"severity":"L2|L3"}}]}},"recommendedQuestionIds":[],"recommendationReason":null}}
...
4. 六维证据分 (0–100)：**只填你能从草稿确认的维度，每题最多 3 个**，其余维度**整个省略，不要填 null 占位**。
   每维必须带 `evidence`（一句话指出草稿哪一行/哪一步支撑这个分）；
   ★ 六维已从报告里下线，它是评分内核的输入，不是给学员看的内容——**不要在六维上展开分析**，
   也不要为了凑满六维去编 evidence。缺维比瞎猜强：评分内核原生支持部分维度。
   `strategyInsight` 若填，另给 `techniqueLevel`（1–5）与 `independentDiscovery`
...
6. `diagnosis`：按量规第五节逐项填实；其中 `walkthrough`（完整解答）按**量规第九节**写、
   `breakpoints`（多断点）按**量规第十节**写——这两节是报告的主轴与右栏，不要当成可选字段跳过。
   `verdict = correct` 的题不要整块填 null，
   ...
   **职责移交（2026-09-14）**：`walkthrough` 是正解的唯一完整处——
   「学员只读这一段不翻解析就能复现动作」这条验收判据改由它承担；
   `fork.standardPath` 不再写正解，只在分叉点写"正确的选择是什么"（≤2 句）；
   `whyDeadEnd` 只讲原理、不讲步骤。
   **去重**：同一式子或事实全题 JSON 不得出现在 3 个以上字段，
   myEntry / fork.myPath / whyDeadEnd / fork.standardPath 与 walkthrough 均不得互为复读。
...
fn get_user_profile(state: State<AppState>) -> Result<UserProfileSettings, String> {
    ...
    let normalized_friend_code = if friend_code.is_empty() {
        ...
        services::friends::sanitize_friend_sync_code(&friend_code).ok()
    };
...
fn set_user_profile(profile: UserProfileSettings, state: State<AppState>) -> Result<(), String> {
    let normalized_friend_code = profile
        .friend_code
        .as_deref()
        .map(services::friends::sanitize_friend_sync_code)
        .transpose()?;
    let conn = state.db.lock().map_err(|e| e.to_string())?;
    conn.execute(
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_user_profile,
            set_user_profile,
            services::friends::test_friend_sync,
            services::friends::publish_friend_snapshot,
            services::friends::pull_friend_snapshots,
            get_daily_plan,
            save_daily_plan,

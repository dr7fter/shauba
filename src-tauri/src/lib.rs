...
// ============================================================================
// 复盘视图命令 (Mistake Review Commands)
// ============================================================================

#[derive(Debug, Deserialize)]
struct MistakeFilters {
...
fn get_mistake_list(
...
) -> Result<Vec<MistakeItem>, String> {
    let conn = state.db.lock().map_err(|e| e.to_string())?;

    // Build dynamic WHERE clause based on filters
    let mut where_conditions = vec!["a.result='wrong'".to_string()];
    let mut params: Vec<Box<dyn rusqlite::ToSql>> = vec![];

    if let Some(chapter) = &filters.chapter {
        where_conditions.push("q.category_path LIKE ?".to_string());
        params.push(Box::new(format!("%{}%", chapter)));
    }

    if let Some(days) = filters.time_range_days {
        where_conditions.push("a.attempted_at >= date('now', '-' || ? || ' days')".to_string());
    ...
    // Check for redone status and apply tag filters
    let mut items = Vec::new();
    for (
        question_id,
        stem,
    ...
    {
        let error_tags: Vec<String> =
            serde_json::from_str(&error_tags_json).unwrap_or_else(|_| vec![]);
        let weakness_tags: Vec<String> =
            serde_json::from_str(&weakness_tags_json).unwrap_or_else(|_| vec![]);
        ...
        if let Some(filter_tags) = &filters.tags {
            let has_matching_tag = filter_tags.iter().any(|filter_tag| {
                error_tags
                    .iter()
                    .any(|t| t.contains(filter_tag) || filter_tag.contains(t))
                    || weakness_tags
                        .iter()
                        .any(|t| t.contains(filter_tag) || filter_tag.contains(t))
            });
        ...
        // Check if this question has been redone correctly
        let (has_redone, redone_at): (bool, Option<String>) = conn
            .query_row(
                "SELECT attempted_at FROM attempts WHERE question_id=?1 AND result='correct' AND attempted_at>?2 ORDER BY attempted_at ASC LIMIT 1",
                params![question_id, attempted_at],
                |r| Ok((true, Some(r.get::<_, String>(0)?))),
            )
            .unwrap_or((false, None));

        items.push(MistakeItem {
            question_id,
            stem,
            correct_answer,
            category_path,
            draft_base64: None, // Draft images not stored in DB currently
            attempted_at,
            duration_seconds,
            ai_rating,
            earliest_error,
...
fn get_report_wall(
    ...
    let query = "
        SELECT 
            ...
            ps.status,
            pr.report_json
        FROM pressure_sessions ps
        LEFT JOIN pressure_reports pr ON pr.session_id = ps.session_id
        WHERE ps.status IN ('graded', 'graded_partial')
    ...
    let cards = stmt
        .query_map([limit], |row| {
            ...
            // Parse report_json to extract summary statistics
            let (question_count, accuracy, avg_rating, avg_duration) = if let Some(ref json_str) =
                report_json
            {
...
#[tauri::command]
fn mark_mistake_redone(
    state: State<AppState>,
    question_id: i64,
) -> Result<(), String> {
    let conn = state.db.lock().map_err(|e| e.to_string())?;

    // Update mastery level: 1 -> 2, 2 -> 3
    conn.execute(
        "UPDATE progress SET mastery = CASE 
            WHEN mastery = 1 THEN 2 
            WHEN mastery = 2 THEN 3 
            ELSE mastery 
        END 
        WHERE question_id = ?1 AND mastery IN (1, 2)",
        params![question_id],
    )
    .map_err(|e| e.to_string())?;

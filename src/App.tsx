...
import {
  ...
  History, Inbox, Layers, LibraryBig, ListPlus, LoaderCircle, Maximize2, Minimize2, Pause, Play, RefreshCw, RotateCcw, ScrollText, Search, Send, Settings, SkipForward,
  Sparkles, Target, ThumbsDown, ThumbsUp, TimerReset, Trash2, X, Zap,
} from 'lucide-react'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import './App.css'
import { localToday, savePracticeSession, loadPracticeSession, clearPracticeSession } from './utils'
import {
...
import { computeGamification, playCorrectSound, playLevelUpSound } from './data/gamification'
...
import type { BootstrapData, CategoryNode, CodexTask, DailyLog, DailyTrendPoint, FailedInboxItem, InboxItem, InsightPoint, MasteryChapter, MasteryNode, Question, QuestionPage, RecommendedQuestion, ReviewHistory, ReviewPlan, WeaknessRadar } from './types'

type View = 'today' | 'library' | 'focus' | 'review' | 'history' | 'inbox' | 'insights' | 'settings'
type AttemptMode = 'paper' | 'review'

const navItems: Array<{ id: View; label: string; icon: typeof BookOpen }> = [
  { id: 'today', label: '今日训练', icon: Zap },
  { id: 'library', label: '题库', icon: LibraryBig },
  { id: 'focus', label: '专项训练', icon: Target },
  { id: 'review', label: '错题复习', icon: TimerReset },
  { id: 'history', label: '每日回顾', icon: History },
  { id: 'inbox', label: 'AI 批改', icon: BrainCircuit },
...
const ratingOptions = [
  ...
  { value: 3, label: '3 · 独立做对', hint: '解题完整 / 略有犹豫' },
...
function App() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    try {
      const [boot, log, map] = await Promise.all([bootstrap(), getDailyLog(), getMasteryMap()])
      setData(boot)
      setDailyLog(log)
  ...
  useEffect(() => {
    ...
    if (session && session.queue.length > 0) {
      setPracticeQueue(session.queue)
      setAttemptMode(session.attemptMode)
      setNotice(`已为你恢复今日未完成练习（剩余 ${session.queue.length} 题）`)
    }
  }, [])

  const gamificationStats = useMemo(() => {
    return computeGamification(dailyLog?.items ?? [], masteryChapters, 0)
  }, [dailyLog, masteryChapters])
  ...
  // Global shortcuts: Alt+F (formula), Alt+D (contract), Alt+B (report), Alt+Z (zen)
  useEffect(() => {
  ...
  return (
    <div className={`app-shell ${isZenMode ? 'zen-mode' : ''}`}>
      <Sidebar view={view} setView={setView} inboxCount={data?.inboxCount ?? 0} />
      <main className="main-area">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={view} className="view-stage" initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: 0.18 }}>
            ...
            {view === 'library' && <LibraryView initialStatus={libraryStatus} queueCount={data?.customQueueCount ?? 0} refresh={refresh} notify={setNotice} onPractice={(questions, reason) => { setPracticeQueue(questions.map((question) => ({ question, score: 100, reason, reasonCode: 'custom' }))); setAttemptMode('paper'); setView('today') }} />}
            {data && view === 'focus' && <FocusView data={data} refresh={refresh} onStart={(queue) => { setPracticeQueue(queue ?? null); setAttemptMode('paper'); setView('today') }} notify={setNotice} />}
            {view === 'review' && (
              <ReviewView
                due={data?.dueCount ?? 0}
            ...
            {view === 'history' && <HistoryView notify={setNotice} onStartVariant={startVariantPractice} />}
            {view === 'inbox' && <InboxView notify={setNotice} onStartRecommendation={startRecommendedBatch} onStartVariant={startVariantPractice} />}
            {data && view === 'insights' && <InsightsView data={data} refresh={refresh} onStart={() => { setPracticeQueue(null); setAttemptMode('paper'); setView('today') }} notify={setNotice} />}
            {data && view === 'settings' && <SettingsView data={data} refresh={refresh} notify={setNotice} />}
          </motion.div>
        </AnimatePresence>
      </main>
...
function Sidebar({ view, setView, inboxCount }: { view: View; setView: (v: View) => void; inboxCount: number }) {
  return (
      ...
      <nav>
        {navItems.map(({ id, label, icon: Icon }) => (
          <button key={id} className={view === id ? 'nav-item active' : 'nav-item'} onClick={() => setView(id)}>
            ...
            <span>{label}</span>
            {id === 'inbox' && inboxCount > 0 && <b>{inboxCount}</b>}
          </button>
        ))}
      </nav>
...
function Topbar({
  ...
  const contract = checkContractStatus(getDailyContract(), data?.todayDone ?? 0, data?.todayMinutes ?? 0)
  ...
  return <header className="topbar">
    ...
    <div className="topbar-meta">
      <button className="topbar-game-btn" onClick={onOpenGameCenter} title="打开考研数一战力与成就中心">
      ...
      <button className={`topbar-dday-btn ${contract.isCompleted ? (contract.claimedReward ? 'done' : 'claimable') : ''}`} onClick={onOpenContract} title="查看/签署今日心流作战契约 (Alt+D)">
        <Target size={14} />
        <span>距初试 <strong>{countdown.days}</strong> 天</span>
        {contract.isCompleted ? (
          contract.claimedReward ? (
            <span className="dday-contract-tag done">✓ 契约达成</span>
          ) : (
            <span className="dday-contract-tag claimable">🎯 契约达成 +60 EXP</span>
          )
        ) : (
          <span className="dday-contract-tag">契约</span>
        )}
      </button>
      <button className="topbar-report-btn" onClick={onOpenBattleReport} title="生成今日数一修炼简报海报 (Alt+B)">
        <ScrollText size={14} />
        <span>战报</span>
        <kbd>Alt+B</kbd>
      </button>
      <button className={`topbar-zen-btn ${isZenMode ? 'active' : ''}`} onClick={onToggleZen} title="切换沉浸专注极简模式 (Alt+Z)">
        {isZenMode ? <Minimize2 size={14} /> : <Maximize2 size={14} />}
        <span>{isZenMode ? '退出专注' : '专注模式'}</span>
        <kbd>Alt+Z</kbd>
      </button>
      <button className="topbar-formula-btn" onClick={onOpenFormula} title="考研数学一公式定理速查 (Alt+F)">
        <BookMarked size={15} />
        <span>公式速查</span>
        <kbd>Alt+F</kbd>
      </button>
      <span><LibraryBig size={15} /> {data?.questionCount ?? 0} 道数一题</span>
      <span><Clock3 size={15} /> 今日 {data?.todayMinutes ?? 0} 分钟</span>
      <span className="local-clock" title="本地时间">{formatLocalClock(now)}</span>
      <div className={timerRunning ? 'focus-timer running' : 'focus-timer'}>
        <Clock3 size={15} />
        <strong>{formatElapsed(elapsedMs)}</strong>
        <button className="timer-control" onClick={toggleTimer} title={timerRunning ? '暂停本次计时' : '开始本次计时'}>{timerRunning ? <Pause size={14} /> : <Play size={14} />}</button>
        <button className="timer-control" onClick={resetTimer} title="清零本次计时"><RotateCcw size={14} /></button>
      </div>
      <button className="icon-button" onClick={onRefresh} title="刷新数据"><RefreshCw size={17} /></button>
    </div>
  </header>
}

function formatLocalClock(time: number) {
  return new Intl.DateTimeFormat('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23' }).format(time)
}
...
function TodayView({
  ...
  }, [current?.id, onActiveQuestionChange])
  ...
  useEffect(() => {
    ...
    setRating(null)
    setTask(null)
  }, [queue[index]?.question.id, resetActiveTimer])
  ...
  const answeredCorrectly = current && current.questionType !== 'subjective' && revealed
  ...
  const submit = async () => {
    ...
    try {
      ...
      const correct = current.questionType === 'subjective' ? rating >= 3 : answeredCorrectly ?? false
      ...
      const durationSeconds = Math.min(1800, Math.max(1, rawSeconds))

      await recordAttempt({ questionId: current.id, durationSeconds, result: correct ? 'correct' : 'wrong', selfRating: rating, selectedAnswer: answer, mode: attemptMode })
  ...
  const reveal = () => { if (!revealed) { setRevealAt(Date.now()); setRevealed(true) } }
  const chooseRating = (value: number) => { if (revealed) setRating(value) }
  ...
  const createTask = async () => {
    if (current) {
      const generated = await createCodexTask(current.id)
      setTask(generated)
      await copyPrompt(generated)
    }
  }
  const createBatchTask = async () => {
    if (queue.length) {
      const generated = await createCodexBatchTask(queue.map((item) => item.question.id))
      setBatchTask(generated)
      await copyPrompt(generated)
    }
  }
  const copyPrompt = async (generated: CodexTask) => {
    await navigator.clipboard.writeText(generated.prompt)
    notify(generated.questionCount > 1 ? '整组批改任务已生成并复制到剪贴板，可直接在手机 Codex 中粘贴' : `批改任务 ${generated.taskId} 已复制到剪贴板，在手机 Codex 发送图片与说明`)
  }
  ...
  if (!current) {
        ...
        <div className="queue-completion-wrap">
          <div className="queue-completion-card">
            ...
            <div className="completion-actions">
              ...
              <button className="secondary-button quiet" onClick={() => setView('focus')}>选择其他考点</button>
            </div>
          </div>
        </div>
      )
    ...
    if (attemptMode === 'review') {
      return (
        <div className="queue-completion-wrap">
          <div className="queue-completion-card">
            <div className="completion-badge review"><Sparkles size={28} /></div>
            <h2>🎉 今日错题复习已全部完成！</h2>
            <p>所有到期需复习巩固的题目已清理完毕，记忆周期已推进。</p>
            <div className="completion-actions">
              <button className="primary-button" onClick={async () => {
                const next = await getRecommendations(12)
                replaceQueue(next)
      ...
      <div className="queue-completion-wrap">
        <div className="queue-completion-card">
          ...
          <div className="completion-actions">
            ...
            <button className="secondary-button" onClick={() => setView('history')}><History size={16} /> 查看今日作答回顾</button>
  ...
  return <div className="today-layout">
    <section className="queue-panel">
      ...
      {data.dailyMode === 'both' && <div className="progress-caption dual"><span><i /> {goalProblemsProgress ?? 0}% 题数</span><span><i /> {goalMinutesProgress ?? 0}% 分钟</span></div>}
      {data.todayDone > 0 && <button className="today-done-pill" onClick={() => setView('history')} title="查看今日已做题目的卡片与 Codex 点评"><History size={13} /><span>今日已做 <b>{data.todayDone}</b> 题（查看回顾）</span></button>}
    ...
    <section className="question-workspace">
      ...
      <article className="question-content">
        ...
        {answeredCorrectly !== null && <div className={answeredCorrectly ? 'judgement correct' : 'judgement wrong'}>{answeredCorrectly ? <><Check size={15} /> 选择与参考答案一致</> : <><X size={15} /> 选择与参考答案不一致，正确答案已在下方标绿</>}</div>}
      </article>
    ...
    <section className="answer-panel answer-panel-inline">
      <div className="answer-head"><div><span>作答与分析</span><small>纸笔优先 · <kbd>1-4</kbd> 快速自评</small></div><span className="autosave"><span /> 自动保存</span></div>
      {!revealed ? <div className="paper-mode">
        ...
        <div className="rating-block"><span>这次做到什么程度？<i>按 1-4 键快速选择</i></span><div className="rating-grid">{ratingOptions.map((item) => <button key={item.value} className={rating === item.value ? 'rating active' : 'rating'} onClick={() => setRating(item.value)}><b>{item.label}</b><small>{item.hint}</small></button>)}</div></div>
        <div className="review-actions">
          <button className="primary-button" disabled={rating === null} onClick={submit}>记录并继续 <ChevronRight size={17} /><kbd>↵</kbd></button>
...
function LibraryView({ initialStatus, queueCount, refresh, notify, onPractice }: { initialStatus: string; queueCount: number; refresh: () => void; notify: (text: string) => void; onPractice: (questions: Question[], reason: string) => void }) {
  ...
  return <div className="library-view">
    <div className="library-toolbar">
      ...
      <div className="segmented">{[['all','全部'],['unseen','未做'],['wrong','错题'],['favorite','收藏'],['noted','有笔记']].map(([value,label]) => <button key={value} className={status === value ? 'active' : ''} onClick={() => setStatus(value)}>{label}</button>)}</div><button className="queue-button" onClick={openQueue}><ListPlus size={17}/><span>训练队列</span><b>{queueCount}</b></button>
    </div>
...
function ReviewView({
  ...
  const selectMode = (next: 'plan' | 'history') => {
    setMode(next)
    setSelectedDate(next === 'plan' ? plan?.days[0]?.date ?? '' : history?.days[history.days.length - 1]?.date ?? '')
    ...
    <div className="review-view">
      ...
      <section className="review-history">
        <div className="review-history-head">
          <div>
            <span>{mode === 'plan' ? '未来 7 天排期' : '最近 7 天完成'}</span>
            <h3>{mode === 'plan' ? '艾宾浩斯复习计划' : '复习完成记录'}</h3>
          </div>
          <div className="review-history-actions">
            ...
            <button className="icon-button" title="刷新复习记录" onClick={load}>
              <RefreshCw size={16} />
            </button>
          </div>
        </div>
                ...
                <div className="review-item-list">
                  {mode === 'plan'
                    ...
                    : (filteredItems as typeof historyItems).map((item) => (
                        <div className="review-slayer-card" key={item.attemptId}>
                          <button
                            className="review-slayer-main"
                            onClick={() => void openQuestion(item.questionId)}
                          >
                            <div className="review-slayer-meta">
                              <span className={item.result === 'correct' ? 'result-dot correct' : 'result-dot wrong'}>
                                {item.result === 'correct' ? '对' : '错'}
                              </span>
                              <b>#{item.questionId}</b>
                              <span className="category-tag">
                                {item.categoryPath.split(' / ').slice(-2).join(' / ')}
...
function HistoryView({ notify, onStartVariant }: { notify: (text: string) => void; onStartVariant: (questionId: number) => void }) {
  ...
  const todayAccuracy = todayStats && todayStats.count > 0 ? Math.round(todayStats.correctCount / todayStats.count * 100) : null

  return <div className="history-view">
    <section className="review-history">
      <div className="review-history-head">
        <div><span>过去每一天</span><h3>每日回顾</h3></div>
        <div className="review-history-actions"><button className="icon-button" title="刷新历史记录" onClick={load}><RefreshCw size={16}/></button></div>
      </div>
      ...
      {todayStats && (
        <div className="history-today-summary">
          ...
          <div>
            ...
            <span>正确率 {todayAccuracy}% · 作答与自评已实时保存</span>
          </div>
        </div>
      )}

      {!log ? <div className="inbox-loading"><LoaderCircle className="spin" size={20}/> 正在读取历史记录</div> : days.length === 0 ? <EmptyState icon={History} title="还没有历史记录" text="完成第一道题后，这里会按天展示你的作答卡片和 Codex 点评。" /> : <><div className="review-days">{days.map((day) => <button key={day.date} className={day.date === selectedDate ? 'review-day active' : 'review-day'} onClick={() => setSelectedDate(day.date)}><span>{day.date.slice(5).replace('-', '/')}</span><strong>{day.count}</strong><small>{day.correctCount}/{day.count} 正确</small></button>)}</div>
      ...
      <div className="history-card-list">{items.map((item) => { const chip = verdictChip(item.aiVerdict); return <article className="history-card" key={`${item.attemptedAt}-${item.questionId}`}>
        <button className="history-card-main" onClick={() => void openQuestion(item.questionId)}>
          <div className="history-card-meta"><span className={item.result === 'correct' ? 'result-dot correct' : 'result-dot wrong'}>{item.result === 'correct' ? '✓' : '✗'}</span><b>#{item.questionId}</b><span>{item.categoryPath.split(' / ').slice(-2).join(' / ')}</span><ChevronRight size={16}/></div>
          <MathText value={item.stem} />
          <div className="history-card-sub-row">
            <small className="history-card-sub">{item.source} · 自评 {item.selfRating}/4 · {new Date(item.attemptedAt).toLocaleString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</small>
        ...
        <div className={item.aiConfirmedAt ? 'history-ai-panel' : 'history-ai-empty'}>
          {item.aiConfirmedAt ? <><div className="history-ai-head"><Sparkles size={14}/><b>Codex 点评</b><i className={`batch-verdict ${chip.tone}`}>{chip.label}</i>{item.aiConfidence !== null && <small>置信度 {Math.round(item.aiConfidence * 100)}%</small>}</div>
  ...
  return <div className="insights-view">
    ...
    {radar && <section className="radar-card">
      ...
      <div className="radar-columns">
        <div className="radar-group"><h4>错误类型</h4>{radar.errorTags.length === 0 ? <div className="empty-mini">还没有 Codex 批改数据</div> : radar.errorTags.slice(0, 8).map((stat) => <div className="radar-row" key={stat.tag}><span>{stat.tag}</span><b>{stat.count}</b><small>近 7 天 {stat.recentCount}</small></div>)}</div>
        <div className="radar-group"><h4>薄弱知识点</h4>{radar.weaknessTags.length === 0 ? <div className="empty-mini">还没有 Codex 批改数据</div> : radar.weaknessTags.slice(0, 8).map((stat) => <div className="radar-row" key={stat.tag}><span>{stat.tag}</span><b>{stat.count}</b><small>近 7 天 {stat.recentCount}</small></div>)}</div>
      </div>
    ...
    {selectedNode && <div className="mastery-selection"><div><span>当前板块</span><b>{selectedNode.path.split(' / ').slice(1).join(' / ')}</b></div><div><span>样本</span><b>{selectedNode.attempted}/{selectedNode.total}</b></div><div><span>正确率</span><b>{selectedNode.accuracy === null ? '--' : `${Math.round(selectedNode.accuracy * 100)}%`}</b></div><div><span>自评</span><b>{selectedNode.rating?.toFixed(1) ?? '--'}</b></div><div><span>掌握分</span><b>{selectedNode.masteryScore === null ? '数据不足' : Math.round(selectedNode.masteryScore)}</b></div><button className="icon-button" title="关闭" onClick={() => setSelectedNode(null)}><X size={16} /></button></div>}
...
function SettingsView({ data, refresh, notify }: { data: BootstrapData; refresh: () => void; notify: (s: string) => void }) {
  const [mode,setMode]=useState(data.dailyMode); const [problems,setProblems]=useState(data.dailyProblemTarget); const [minutes,setMinutes]=useState(data.dailyMinuteTarget)
  const [intervals,setIntervals]=useState(data.reviewIntervals.length === 4 ? data.reviewIntervals : [1,3,7,15])
  const [exporting,setExporting]=useState(false)
  const submit=async()=>{await saveGoal({dailyMode:mode,dailyProblemTarget:problems,dailyMinuteTarget:minutes});notify('训练目标已保存');refresh()}
  const saveIntervals=async()=>{await saveReviewIntervals(intervals);notify('复习间隔已保存');refresh()}
  const exportNow=async()=>{setExporting(true);try{const result=await exportRecords();notify(`备份完成：${result.dbPath}`)}catch(error){notify(`备份失败：${String(error)}`)}finally{setExporting(false)}}
  return <div className="settings-view"><section><div><h2>每日训练目标</h2><p>题数和时长可以同时启用，完成任一项不会强制结束训练。</p></div><div className="setting-control"><label>目标模式</label><div className="segmented wide">{[['problems','固定题数'],['minutes','固定时长'],['both','两种都看']].map(([v,l])=><button key={v} className={mode===v?'active':''} onClick={()=>setMode(v as typeof mode)}>{l}</button>)}</div></div><div className="numeric-settings"><label>每日题数<input type="number" min="1" max="200" value={problems} onChange={e=>setProblems(Number(e.target.value))}/></label><label>每日分钟<input type="number" min="5" max="600" value={minutes} onChange={e=>setMinutes(Number(e.target.value))}/></label></div><button className="primary-button compact" onClick={submit}>保存设置</button></section><section><div><h2>复习间隔</h2><p>四档自评对应的下次复习天数。冲刺阶段可以压缩到更短节奏。</p></div><div className="numeric-settings four"><label>不会<input type="number" min="1" max="180" value={intervals[0]} onChange={e=>setIntervals([Number(e.target.value),intervals[1],intervals[2],intervals[3]])}/></label><label>模糊<input type="number" min="1" max="180" value={intervals[1]} onChange={e=>setIntervals([intervals[0],Number(e.target.value),intervals[2],intervals[3]])}/></label><label>会做<input type="number" min="1" max="180" value={intervals[2]} onChange={e=>setIntervals([intervals[0],intervals[1],Number(e.target.value),intervals[3]])}/></label><label>熟练<input type="number" min="1" max="180" value={intervals[3]} onChange={e=>setIntervals([intervals[0],intervals[1],intervals[2],Number(e.target.value)])}/></label></div><button className="primary-button compact" onClick={saveIntervals}>保存间隔</button></section><section><div><h2>本地数据与备份</h2><p>大观园题库源只读；学习记录保存在本机数据库。建议每周导出一次备份，换机或重装系统时可以用。</p></div><dl><dt>大观园题库</dt><dd>{data.libraryDir}</dd><dt>学习数据库</dt><dd>{data.dataDir}</dd><dt>Codex 收件箱</dt><dd>{data.inboxDir}</dd></dl><div className="setting-actions"><button className="primary-button compact" disabled={exporting} onClick={exportNow}>{exporting ? <LoaderCircle className="spin" size={15}/> : <Archive size={15}/>} 导出学习记录（数据库 + JSON）</button><span>备份文件会写入 {data.dataDir} 下的 backups 目录</span></div></section><section><div><h2>隐私与 AI</h2><p>刷题、推荐和复习可以完全离线。只有你主动把草稿发给 Codex 时，图片才会进入对应的 Codex 任务。</p></div><div className="privacy-line"><span className="status-dot"/><span>本地优先模式已启用</span></div></section></div>
}
...
function Toast({ text, close }: { text: string; close: () => void }) { useEffect(()=>{const t=setTimeout(close,3500);return()=>clearTimeout(t)},[close]); return <motion.div className="toast" initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} exit={{opacity:0,y:8}}><Check size={16}/><span>{text}</span><button onClick={close}><X size={15}/></button></motion.div> }

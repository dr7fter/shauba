...
export function InsightsView({
  ...
  const loadTactical = async (scope: ScopeMode, seasonStart: string | null) => {
    try {
      const stats = await getTacticalDashboardStats(scope, seasonStart)
  ...
  // 完美平台式赛季筛选：默认当前赛季，可切历史赛季或全部生涯
  const effectiveSeasonStart =
    seasonPick === 'all'
      ? null
      : seasonPick === 'current'
      ? seasonLadder?.weekStart ?? null
      : seasonPick
  useEffect(() => {
    void loadTactical(scopeMode, effectiveSeasonStart)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scopeMode, effectiveSeasonStart])
    ...
    <div className="insights-view data-view tactical-dashboard-view">
      <div className="insights-tabs data-tabs tactical-nav-tabs">
        ...
        <div className="tactical-top-controls">
          ...
          {tab === 'overview' && (
            <select
              className="season-selector-chip"
              value={seasonPick}
              onChange={(e) => setSeasonPick(e.target.value)}
              title="周赛季制：每周一 00:00 开启，周日晚 24:00 自动结算"
              style={{ appearance: 'auto', padding: '5px 10px', fontSize: 12.5, fontWeight: 600, color: 'var(--ink)', cursor: 'pointer' }}
            >
              <option value="current">当前赛季{seasonLadder ? `（周 ${seasonLadder.weekStart} 起）` : ''}</option>
              {(seasonLadder?.seasons ?? [])
                .filter((season) => season.weekStart !== seasonLadder?.weekStart)
                .map((season) => (
                  <option key={season.weekStart} value={season.weekStart}>
                    周 {season.weekStart}
                  </option>
                ))}

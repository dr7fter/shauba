...
function ObjectiveCard({ objective, onOpen }: { objective: LearningObjective; onOpen: () => void }) {
  return <article className={'learning-objective learning-objective-' + objective.status}>
    ...
    <h3>{objective.title}</h3><p>{objective.whyNow || '暂无推荐理由。'}</p>
    ...
    <div className="learning-objective-footer"><small>{objective.blockedReason ?? objective.successCriteria}</small><button type="button" className="learning-link-button" onClick={onOpen}>{objective.status === 'stable_completed' ? '查看闭环' : '打开入口'} <ArrowRight size={14} /></button></div>
  </article>
}
...
export function LearningCenterView({ initialData = null, featureFlags, onNavigate, onNotify, onRefresh }: LearningCenterViewProps) {
  ...
  const openChain = (chain: MistakeChain) => onNavigate({ type: 'review', mistakeChainId: chain.id, questionId: null })
